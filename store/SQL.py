class SQLStore(object):
    ...
    def save(self, obj):
        ...
        val_list = [obj_dict[fld] for fld in obj_dict if self.mapping.object_to_db(cls.__name__, fld) is not None]
        ...
        cur = self.db.execute(qry, val_list)
        self.db.commit()
        return cur
    ...
    def createInsertQuery(self, obj):
        if self.mapping is None:
            ...
            fields = [self.mapping.object_to_db(cls.__name__, fld) for fld in obj.get_dict() if self.mapping.object_to_db(cls.__name__, fld) is not None]
            qmarks = ['?' for fld in fields]
